...
//   return subject;
 ...
 saveEvent(event) {
    console.log(' formValues in save event => ', event);
    event.id= 999;
    event.sessions = [];
 ...
 searchSessionByName(searchText: string) {
   console.log('searchSessionByName in event.service => 3',searchText);
    let text = searchText.toLowerCase();
    let filteredSession: ISession[] = [];
    try {
        EVENTS.forEach(function(event){    
            console.log('in for each of EVENTS');
            let matchingSessions = event.sessions.filter(function(session){
              return session.name.toLocaleLowerCase().indexOf(text) > -1;
            });

            matchingSessions = matchingSessions.map(function(session: any){
             session.eventId = event.id;          
             return session;
            });
            console.log('matchingSessions = >',matchingSessions);
            filteredSession = filteredSession.concat(matchingSessions);         
        });
    ...
    setTimeout(function(){      
      console.log('setTimeout in event,.service 4 =>',filteredSession);    
      eventEmitter.emit(filteredSession);
    }, 100);
    return eventEmitter; 
 }
